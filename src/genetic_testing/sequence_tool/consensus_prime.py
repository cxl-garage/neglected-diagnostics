...
parser.add_argument(
    "-o", "--outdir", type=str, help="Name of the output directory.", default=""
)
...
parser.add_argument(
    ...
    "--threads",
    type=str,
    help="Number of threads used by MAFFT. Default = -1 (all)",
    default=-1,
)
parser.add_argument(
    "-k",
    "--keepduplicates",
    type=str2bool,
    help="Keep duplicate sequences. Default = False",
    default=False,
)
parser.add_argument(
    "-c",
    "--consensusthreshold",
    type=float,
    help="Consensus threshold between 0 and 1 with 1 being a perfect consensus for regions to be considered for primer prediction in the final alignment. Default = 0.95",
    default=0.95,
)
...
parser.add_argument(
    ...
    "--consensussimilarity",
    type=float,
    help="Minimum similarity threshold for sequences in the input alignment when comparing each sequence to the consensus sequence. Default = 0.8",
    default=0.8,
)
...
parser.add_argument(
    "--primers",
    type=str,
    help="Known primers for visualization in primer plot in .fasta format.",
    default="",
)
parser.add_argument(
    "--negativesequences",
    type=str,
    help="File with sequences that get their consensus sequence added to the final alignment .fna.",
    default="",
)
...
def insert_newlines(
    string, linelen=64
):  # linelen= 64 is recommended for a well readable file.
...
def align(infile, outfile):
    with open(outfile, "w") as outfile:
        subprocess.call(
            ["mafft", "--auto", "--adjustdirection", "--thread", threads, infile],
            stdout=outfile,
...
def to_html_table(table, header=[]):
    ...
    if header:
        html_table += (
            "<tr><th>" + "</td><th>".join([str(x) for x in header]) + "</th></tr>\n"
        )
    for row in table:
        html_table += (
            "<tr><td>" + "</td><td>".join([str(x) for x in row]) + "</td></tr>\n"
        )
    html_table += "</table>\n"
    return html_table


# returns the percentage similarity of two sequency as value between 0 and 1
...
if not keepduplicates:
    ...
    if number_of_uniques > 1:
        align(
            infile=f"{outdir}/unique_fasta.fna",
            outfile=f"{outdir}/unique_alignment.fna",
        )
    else:
...
print(
    f"Building and writing similarity filtered alignment to: {outdir}/{unique}similarity_filtered_alignment.fna\n"
)
if len(similarity_filtered_dict) > 1:
    align(
        infile=f"{outdir}/{unique}similarity_filtered_fasta.fna",
        outfile=f"{outdir}/{unique}similarity_filtered_alignment.fna",
    )
else:
    os.system(
        f"cp {outdir}/{unique}similarity_filtered_fasta.fna {outdir}/{unique}similarity_filtered_alignment.fna"
    )

# read similarity_filtered_alignment fasta
fasta, fastaheader = read_fasta(
    f"{outdir}/{unique}similarity_filtered_alignment.fna", delim, idpos
)
number_of_sequences = len(fasta)
...
with open(outfile, "w") as outfile:
    ...
    outfile.write(
        f'1\t{len(consensus_seq_gapless)}\t{len(consensus_seq_gapless)}\t{"{:.3f}".format(avg(consensus_score))}\t{consensus_seq_gapless}\n'
    )
    ...
    for region in consensus_regions:
        ...
        print(
            f'Start: {region[0]+1} Stop: {region[1]+1} Length: {region[1]+1 - region[0]} Consensus_score: {"{:.3f}".format(score_avg)} Sequence: {consensus_seq_gapless[region[0]:region[1]+1]}'
        )
        outfile.write(
            f'{region[0]+1}\t{region[1]+1}\t{region[1]+1 - region[0]}\t{"{:.3f}".format(score_avg)}\t{consensus_seq_gapless[region[0]:region[1]+1]}\n'
        )
#    print(scores)
...
excluded_regions_out = " ".join(
    [",".join([str(y) for y in x]) for x in excluded_regions]
)
...
if negativesequencefile:
    negativesequences, negativesequence_header = read_fasta(
        negativesequencefile, delim, idpos
    )
...
with open(f"{outdir}/consensus_alignment.fna", "w") as outfile:
    subprocess.call(
        [
            ...
            threads,
            "--add",
            f"{outdir}/consensussequences_fasta.fna",
            f"{outdir}/{unique}similarity_filtered_alignment.fna",
        ],
        stdout=outfile,
        stderr=subprocess.DEVNULL,
    )
...
with open(f"{outdir}/consensusregions_alignment.fna", "w") as outfile:
    subprocess.call(
        [
            ...
            threads,
            "--addfragments",
            f"{outdir}/consensusregions_fasta.fna",
            f"{outdir}/consensus_alignment.fna",
        ],
        stdout=outfile,
        stderr=subprocess.DEVNULL,
    )
    ...
    with open(f"{outdir}/final_alignment.fna", "w") as outfile:
        subprocess.call(
            [
                ...
                "--adjustdirection",
                "--addfragments",
                f"{outdir}/primer_fasta.fna",
                f"{outdir}/consensusregions_alignment.fna",
            ],
            stdout=outfile,
            stderr=subprocess.DEVNULL,
        )
...
with open(html_file, "w") as outfile:
    ...
    for param in primer3_dict:
        ...
        if str(primercount) in param or str(primercount + 1) in param:
            ...
            if re.match("PRIMER_LEFT_.+_SEQUENCE", param):
                ...
                primer_resulttable.append(
                    [param + "_REV", revcomp(primer3_dict[param])]
                )
                primer_resulttable.append(
                    [param + "_REVCOMP (5'-3')", primer3_dict[param]]
                )
            else:
            ...
            if param == "SEQUENCE_TEMPLATE":
                primer3_dict[param] = (
                    "<pre>"
                    + insert_newlines(primer3_dict[param], linelen=100)
                    + "</pre>"
                )
    ...
    if negativesequencefile:
        consensusparametertable.append(
            ["Sequences for negative consensus alignment", negativesequencefile]
        )
    # write HTML document
    outfile.write("<html>\n<head>\n<style>\n")
    ...
    outfile.write(
        "th { border: 1px solid #909090; text-align: center; padding: 8px; }\n"
    )
    ...
    outfile.write(
        to_html_table(
            alignment_statistic_table, header=["Fasta/Alignment", "Number of Sequences"]
        )
    )
    outfile.write("<br><hr><br>\n")
    ...
    if primer_dict:
        for primerID in primer_resulttables:
            ...
            outfile.write(
                to_html_table(
                    primer_resulttables[primerID], header=["Parameter", "Value"]
                )
            )
    else:
